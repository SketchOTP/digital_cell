...
7. `mimir_retrieval_feedback(session_id, useful_memory_ids_json, missing?)` before close with only memory IDs that actually helped (`[]` if none); never invent usefulness.
8. `mimir_task_close(task_id, version, status, changed_files_json, tests_json, lessons_json)` only after evidence inspection, using the latest version and verified result.
...
## Mandatory retrieval feedback

After every `mimir_context_compile`, retain `retrieval.session_id` and item `memory_id` values; before `mimir_task_close`, call `mimir_retrieval_feedback` with only IDs that actually helped (`[]` if none) and an optional missing-context note. Never invent usefulness; do not rely on dashboard rating when MCP is available.
