...
- `digital-protocell/examples/dcfinal001_r4_evolution.rs` — R12 observer-only native force extraction, route-separated discarded-clone susceptibility probes, zero-force baseline, force-cap normalization and state-normalized response metrics; no production route or reproduction stage is enabled.
- `digital-protocell/examples/dcm4_r12_state_normalized_p_to_m_force_response_requalification.rs` — R12 authority/contract seal, exact thirty-state runner and downstream-stop assertions.
- `digital-protocell/experiments/dcm4_r12_state_normalized_p_to_m_force_response_requalification_verify.py` — independent force provenance, native parity, signed two-scale susceptibility, state-control-envelope and terminal-predicate verifier.
