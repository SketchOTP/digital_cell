...
impl LineageTracker {
    ...
    pub fn register_offspring(
        ...
        let parent = self
            .ancestry
            .get(&parent_id)
            .ok_or(LineageError::MissingParent(parent_id))?;
        if birth_event_id == 0 {
            return Err(LineageError::InvalidBirthEvent(organism_id));
        }
        self.ancestry.insert(
    ...
    pub fn record_observation(
        ...
        let record = self
            .ancestry
            .get_mut(&organism_id)
            .ok_or(LineageError::MissingParent(organism_id))?;
    ...
    pub fn record_death(&mut self, organism_id: OrganismId, time: f64) -> Result<(), LineageError> {
        let record = self
            ...
            .get_mut(&organism_id)
            .ok_or(LineageError::MissingParent(organism_id))?;
        if record.death_time.is_some() {
            return Err(LineageError::DoubleDeath(organism_id));
        }
    ...
    pub fn descendant_depth(&self, organism_id: OrganismId) -> u32 {
        self.children(organism_id)
            .into_iter()
            .map(|child| 1 + self.descendant_depth(child))
            .max()
            .unwrap_or(0)
    }
    ...
    pub fn lineage_depth(&self, organism_id: OrganismId) -> u32 {
        self.descendant_depth(organism_id)
    }
...
pub struct GenerationTracker {
    ...
    pub generation_intervals: Vec<GenerationIntervalV1>,
    pub founder_time: Option<f64>,
}

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
pub struct GenerationIntervalV1 {
    pub parent_id: OrganismId,
    pub parent_generation: u32,
    pub parent_birth_time: f64,
    pub fission_time: f64,
    pub generation_duration: f64,
}

impl GenerationTracker {
    pub fn record_founder(&mut self, time: f64) {
        self.founder_time = Some(time);
    }

    pub fn record_completed_fission(
        &mut self,
        parent_id: OrganismId,
        parent_generation: u32,
        parent_birth_time: f64,
        time: f64,
    ) {
        ...
        *self.generation_distribution.entry(generation).or_default() += 2;
        self.time_to_first_birth
            .get_or_insert_with(|| time - self.founder_time.unwrap_or(time));
        let duration = (time - parent_birth_time).max(0.0);
        self.generation_times.push(duration);
        self.generation_intervals.push(GenerationIntervalV1 {
            parent_id,
            parent_generation,
            parent_birth_time,
            fission_time: time,
