use crate::{
    ...
    EnvironmentContext, EnvironmentProtocolV1, FounderIdentityV1, Metadata, OrganismAdapter,
};
...
impl Default for DigitalCellMeshAdapter {
    fn default() -> Self {
        Self {
            mech: MechParams::default(), reactions: ReactionParams::default(), transport: TransportParams::default(),
            growth: GrowthParams::default(), fission: FissionParams::default(), enable_mechanics: true,
            enable_fission: true, founder_radius: 14.0,
        }
    }
}
...
impl OrganismAdapter for DigitalCellMeshAdapter {
    ...
    fn accepted_dt(&self) -> f64 { self.mech.dt }
    ...
    fn apply_declared_environment(
        ...
        if matches!(environment.resource_mode, crate::ResourceMode::Continuous) {
            organism.exterior.n = (organism.exterior.n + ecology.continuous_supply.n * context.accepted_dt).max(0.0);
            organism.exterior.f = (organism.exterior.f + ecology.continuous_supply.f * context.accepted_dt).max(0.0);
            organism.exterior.w = (organism.exterior.w + ecology.continuous_supply.w * context.accepted_dt).max(0.0);
        }
        let on_pulse = environment.pulse_schedule.iter().chain(ecology.pulse_schedule.iter()).any(|time| (*time - accepted_simulated_time).abs() <= context.accepted_dt * 0.5 + f64::EPSILON);
        ...
        if matches!(environment.resource_mode, crate::ResourceMode::Scarcity) {
            let scarce = environment.scarcity_schedule.iter().chain(ecology.scarcity_schedule.iter()).any(|window| accepted_simulated_time >= window.start && accepted_simulated_time < window.end);
        ...
        let damage_mode = if environment.damage_mode != crate::DamageMode::None { &environment.damage_mode } else { &ecology.damage_mode };
        if *damage_mode != crate::DamageMode::None {
            if let Some(interval) = environment.damage_interval.or(ecology.damage_interval) {
                let crossed = (accepted_simulated_time / interval).floor() < ((accepted_simulated_time + context.accepted_dt) / interval).floor();
                if interval > 0.0 && crossed {
                    let fraction = if ecology.damage_fraction > 0.0 { ecology.damage_fraction } else { 0.05 };
                    ...
                    events.push(AdapterEnvironmentEvent { event_type: crate::EventType::DamageApplied, metadata });
                }
            }
        }
    ...
    fn advance(&mut self, organism: &mut Self::Organism, _environment: &EnvironmentProtocolV1, _accepted_step: u64, _accepted_simulated_time: f64) -> Result<AdvanceOutcome<Self::Organism>, AdapterError> {
        if !organism.alive { return Ok(AdvanceOutcome::Died { reason: organism.death_reason.clone().unwrap_or_else(|| "mesh_derived_dead".into()), accepted_dt: self.mech.dt, metadata: Metadata::new() }); }
        let (_, _, split) = coupled_step_growth(organism, &self.mech, &self.reactions, &self.transport, &self.growth, &self.fission, self.enable_mechanics, self.enable_fission);
        if let Some((daughter_a, daughter_b, event)) = split {
            ...
            return Ok(AdvanceOutcome::Fission { offspring: vec![daughter_a, daughter_b], accepted_dt: self.mech.dt, metadata });
        }
        if !organism.alive { return Ok(AdvanceOutcome::Died { reason: organism.death_reason.clone().unwrap_or_else(|| "mesh_derived_dead".into()), accepted_dt: self.mech.dt, metadata: Metadata::new() }); }
        Ok(AdvanceOutcome::Continuing { accepted_dt: self.mech.dt, metadata: Metadata::new() })
    }

    fn is_alive(&self, organism: &Self::Organism) -> bool { organism.alive }
    fn phenotype(&self, organism: &Self::Organism) -> String { format!("mesh_vertices:{};mass:{:.9};area:{:.9}", organism.n(), organism.total_structural_mass(), organism.area()) }
    fn hereditary_state(&self, organism: &Self::Organism) -> String { format!("equation:{};templates:{};autocatalytic_edges:{}", organism.equation_id, organism.templates.len(), organism.autocatalytic_edges.len()) }
}
