...
pub fn run_v4_stage_b(output: &Path, eps_m: f64) -> Result<Value, Box<dyn std::error::Error>> {
    d008::run_stage_b_with(output, &v4_stage_options(eps_m))
}

pub fn run_v4_stage_c(output: &Path, eps_m: f64) -> Result<Value, Box<dyn std::error::Error>> {
    d008::run_stage_c_with(output, &v4_stage_options(eps_m))
}
