use chemistry_core::autocatalytic_copying::{
    founder_h_edges, founder_n_edges, seed_founder_edges,
};
use chemistry_core::autocatalytic_nodes::{
    autocatalytic_schema_load_ok, stamp_autocatalytic_equation, AutocatalyticParams, MU_E,
...
fn partition_spreads_edges() {
    use chemistry_core::autocatalytic_copying::{founder_h_edges, redistribute_edges_along_axis, seed_founder_edges};
    ...
    let mut mesh = MaterialMesh::seed_regular(
        24, 12.0, 40.0, 40.0, DEFAULT_RHO_S, 0.7,
        LumpedChem { c: 0.8, a: 1.0, n: 0.5, f: 0.5, r: 0.7, q_k: 0.5, q_e: 0.5, k_a: 0.15, k_r: 0.15, k_node_b: 0.15, ..Default::default() },
        LumpedChem { n: 1.5, f: 1.5, ..Default::default() },
        5.0,
    );
    let c = mesh.centroid();
    for v in &mut mesh.vertices { v[0] = c[0] + (v[0]-c[0])*1.45; }
    stamp_autocatalytic_equation(&mut mesh);
    seed_founder_edges(&mut mesh, &founder_h_edges());
    ...
    let mut got = None;
    for s in 0..12000 {
        mesh.exterior.n = 1.5; mesh.exterior.f = 1.5;
        ...
        remesh(&mut mesh);
        if s % 50 == 0 { redistribute_edges_along_axis(&mut mesh); }
        if mesh.total_structural_mass() >= 1.35*birth && s % 10 == 0 {
            ...
            if let Some((d1,d2,ev)) = try_local_fission(&mesh, &fission) {
                let c1=d1.centroid(); let c2=d2.centroid();
                eprintln!("pinch {:?} d1n={} d2n={} c1={:?} c2={:?}", ev.pinch, d1.n(), d2.n(), c1, c2);
                ...
                eprintln!("d1 edges {} mass {} d2 edges {} mass {}", d1.autocatalytic_edges.len(), d1.total_structural_mass(), d2.autocatalytic_edges.len(), d2.total_structural_mass());
                got = Some((d1,d2));
                break;
            }
        }
    ...
    assert!(d1.autocatalytic_edges.len() > 0 && d2.autocatalytic_edges.len() > 0,
        "expected both daughters to get edges, got {} and {}", d1.autocatalytic_edges.len(), d2.autocatalytic_edges.len());
}


#[test]
fn selection_completes_generations() {
    use chemistry_core::autocatalytic_copying::{founder_h_edges, founder_b_edges, seed_founder_edges, redistribute_edges_along_axis};
    ...
    let mut pop = MeshPopulation::seed_one(14.0, 11, 2.2);
    {
        ...
        let c = mesh.centroid();
        for v in &mut mesh.vertices { let dx = v[0]-c[0]; v[0] = c[0] + dx*1.35; }
        stamp_autocatalytic_equation(mesh);
        mesh.interior.r = 0.6; mesh.interior.a = 0.9;
        mesh.interior.q_k = 0.5; mesh.interior.q_e = 0.5;
        mesh.interior.k_a = 0.12; mesh.interior.k_r = 0.12; mesh.interior.k_node_b = 0.12;
        seed_founder_edges(mesh, &founder_h_edges());
        pop.individuals[0].birth_mass = mesh.total_structural_mass();
    }
    let _ = (founder_b_edges,);
    let mut react = ReactionParams::default();
    react.reserve = ReserveParams::derived(80.0, 40.0, 0.5, 0.3, 2.0, 0.1, pop.individuals[0].mesh.area());
    react.reserve.enable = true;
    react.autocatalytic = AutocatalyticParams::derived(40.0).with_mutation_off();
    ...
    let mut pulse = PulseLeanState::new(PulseLeanSchedule { cycle_period: period, pulse_fraction: 0.35, cycle_nf_budget: 1.10*0.05*period, lean_nf_rate: 0.0 });
    let rich = 2.2;
    for s in 0..18000 {
        let (n, f) = if pulse.in_pulse() { (rich*1.25, rich*1.25) } else { (rich*0.12, rich*0.12) };
        for ind in &mut pop.individuals {
            if ind.mesh.alive { ind.mesh.exterior.n = n; ind.mesh.exterior.f = f; }
        }
        pulse.t += mech.dt;
        if s % 40 == 0 {
            for ind in &mut pop.individuals { if ind.mesh.alive { redistribute_edges_along_axis(&mut ind.mesh); } }
        }
        let _ = pop.step(&mech, &react, &transport, &growth, &fission, true);
        if pop.living_count() == 0 || pop.living_count() > 32 { break; }
    }
    let max_gen = pop.individuals.iter().map(|i| i.generation).max().unwrap_or(0);
    eprintln!("selection probe max_gen={} alive={} fissions={}", max_gen, pop.living_count(), pop.fission_log.len());
    assert!(max_gen >= 2, "expected multi-gen under repaired H ecology, got gen {max_gen} fissions {}", pop.fission_log.len());
}
