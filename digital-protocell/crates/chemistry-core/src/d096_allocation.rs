...
pub const EQUATION_VERSION_FINITE_CATALYTIC_ALLOCATION_V4: &str =
    "autopoietic_material_mesh_finite_catalytic_allocation_v4_finite_budget_centered_gain";
pub const FINITE_ALLOCATION_V4_SCHEMA_VERSION: u32 = 5;
...
impl AllocationGenotype {
    ...
    pub fn candidate_hash_v4(self, params: &AllocationParams) -> String {
        let mut bytes = EQUATION_VERSION_FINITE_CATALYTIC_ALLOCATION_V4
            .as_bytes()
            .to_vec();
        bytes.extend_from_slice(&FINITE_ALLOCATION_V4_SCHEMA_VERSION.to_le_bytes());
        for value in self.0 {
            bytes.extend_from_slice(&value.to_le_bytes());
        }
        for value in [
...
pub fn allocation_v4_schema_load_ok(mesh: &MaterialMesh, params: &AllocationParams) -> bool {
    mesh.equation_id == EQUATION_VERSION_FINITE_CATALYTIC_ALLOCATION_V4
        && mesh.schema_version == FINITE_ALLOCATION_V4_SCHEMA_VERSION
        && mesh
            .finite_allocation
            .is_some_and(|state| state.genotype.valid(params))
}
...
/// D096-v4 preserves the complete D096-v3 activated-material expression law.
/// Its only versioned change is the finite-budget centered interpretation in
/// [`function_gain`]; physical catalyst stocks and mutation remain unchanged.
pub fn expression_step_activated_material_v4(
    mesh: &mut MaterialMesh,
    params: &AllocationParams,
    dt: f64,
) -> Result<ExpressionLedger, ExpressionReject> {
...
/// D096-v4 finite-budget centered intensive gain.
///
/// The catalyst concentrations compete inside the fixed four-function budget:
/// equal allocation is neutral, while positive excess in one function is
/// balanced by reduced gains elsewhere. The expression is scale invariant and
/// sums to FUNCTIONS for every nonnegative concentration vector.
pub fn finite_budget_centered_gain(concentrations: [f64; FUNCTIONS], index: usize) -> f64 {
    assert!(index < FUNCTIONS);
    let total = concentrations.iter().sum::<f64>();
    1.0 + ((FUNCTIONS as f64) * concentrations[index] - total) / (0.1 + total)
}

pub fn function_gain(mesh: &MaterialMesh, index: usize) -> f64 {
    mesh.finite_allocation
        .map(|state| {
            if mesh.equation_id == EQUATION_VERSION_FINITE_CATALYTIC_ALLOCATION_V4
                && mesh.schema_version == FINITE_ALLOCATION_V4_SCHEMA_VERSION
            {
                let area = mesh.area();
                assert!(
                    area.is_finite() && area > 0.0,
                    "D096-v4 gain requires positive physical area"
                );
                let concentrations = state.catalysts.map(|catalyst| catalyst.max(0.0) / area);
                finite_budget_centered_gain(concentrations, index)
            } else if mesh.equation_id == EQUATION_VERSION_FINITE_CATALYTIC_ALLOCATION_V3
                && mesh.schema_version == FINITE_ALLOCATION_V3_SCHEMA_VERSION
            {
                let area = mesh.area();
                assert!(
