//! DC-FINAL-001-R3: maturation-safe D-096 expression and frozen evolution replay.
//!
...
fn main() {
    let mut output = PathBuf::from("/tmp/dcfinal001_r3_evolution.json");
    ...
    assert!(!exact_r2_gate6_pass, "unexpected exact R2 gate-6 pass");
    let value = json!({
        "directive": DIRECTIVE,
        "protocol": {
        ...
        "gate6_exact_r2_newborn_continuity": "FAIL",
        "evolution_execution": "NOT_RUN_STOPPED_AT_GATE6",
        "classification": "D096_V4_STRUCTURAL_TRANSFER_SEMANTICS_INVALID",
        "causal_boundary": "The authorized V4-only proportional repair is valid on V4 daughters, but the sealed R2 evolution template is HistoricalV1. Applying the repair to exact R2 would require a non-V4 semantic change; converting R2 to V4 would violate exact frozen replay.",
    });
