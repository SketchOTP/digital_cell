...
use chemistry_core::mesh_reactions::{reactions_step, reactions_step_with_reserve_mode, ReactionParams, ReserveDiagnosticMode};
use chemistry_core::mesh_topology::TopologyLedger;
use chemistry_core::mesh_transport::{transport_step, TransportParams};
use regulatory_core::{apply_local_activated_energy_contractility_with_stick_slip, apply_stick_slip_to_legacy_mechanics, ContractilityParamsV1, StickSlipTractionParamsV1, FROZEN_ZERO_MOTION_TOLERANCE};
...
const DIRECTIVE: &str = "DC-DEV-021-M2-ENTRY-022-POST-FISSION-INHERITED-POLARITY-TRANSIENT-LOCOMOTION-FEASIBILITY-001";
const START: &str = "7a372cff217c1ce4af78a5c5dacada761d7e41d7";
const DT: f64 = 0.02;
...
fn motor_range(s: &AmountState) -> f64 {
    s.u.iter()
        .zip(&s.v)
        .map(|(u, v)| {
            assert!(*u + *v > 0.0, "motor range requested before interface eligibility");
            u / (u + v)
        })
...
fn passive_post_fission_step(mesh: &mut MaterialMesh) -> (Vec<f64>, Vec<f64>, usize) {
    ...
    let transport = TransportParams::default();
    let growth = GrowthParams { y_g: 0.9, enable_growth: false };
    ...
    remesh(mesh);
    let origin = mesh.vertices.first().and_then(|new_first| {
...
fn eligible_state(mesh: &MaterialMesh, g: &Grid, state: &AmountState) -> (MaterialMesh, Grid, AmountState, Value) {
    ...
    let min_pool = eligible.u.iter().zip(&eligible.v).map(|(u, v)| u + v).fold(f64::INFINITY, f64::min);
...
fn run_transient(mesh_start: &MaterialMesh, g: Grid, state_start: AmountState, arm: &str, uniform: bool, motor_off: bool) -> TransientRun {
    let mechanics = MechParams::default();
    ...
    let initial_radius = mesh.vertices.iter().map(|p| vector_norm(vector_sub(*p, initial_centroid))).fold(0.0, f64::max);
    ...
    for step in 1..=MOTOR_STEPS {
        let old_grid = current_grid.clone();
        let _reaction = reactions_step_with_reserve_mode(&mut mesh, &reaction_params, mechanics.dt, true, true, ReserveDiagnosticMode::Full);
        let fractions = exact_active_fraction(&state);
        let mean = fractions.iter().sum::<f64>() / fractions.len() as f64;
        let motor = if motor_off { vec![0.0; mesh.n()] } else if uniform { vec![mean; mesh.n()] } else { fractions };
        ...
        if step == 1 { first_motor = (min_motor, max_motor); first_range = range; }
        ...
        let ledger = if motor_off {
            ...
            let l = apply_local_activated_energy_contractility_with_stick_slip(&mut mesh, &motor, &mechanics, &contractility, &traction).unwrap();
            slips += l.slipping_contacts;
            stuck += l.stuck_contacts;
            if let Some(c) = l.contractility.as_ref() {
                if c.requested_resource > c.resource_spent + NUM_TOL { a_limited_steps += 1; }
                a_to_w_residual = a_to_w_residual.max((c.activated_amount_before - c.activated_amount_after + c.waste_amount_before - c.waste_amount_after).abs());
                traction_difference += c.maximum_tension.abs();
                a_spent += c.resource_spent;
            }
            Some(l)
        };
        let reaction = reactions_step_with_reserve_mode(&mut mesh, &reaction_params, mechanics.dt, true, true, ReserveDiagnosticMode::Full);
        w_generated += reaction.w_produced;
        let before_vertices = mesh.vertices.clone();
        remesh(&mut mesh);
        let origin = mesh.vertices.first().and_then(|first| before_vertices.iter().position(|old| (old[0]-first[0]).hypot(old[1]-first[1]) <= 1e-9)).unwrap_or(0);
        let new_lengths: Vec<f64> = (0..mesh.n()).map(|i| mesh.edge_length(i)).collect();
        let new_grid = grid(&new_lengths);
        ...
        max_envelope = max_envelope.max(mesh.vertices.iter().map(|p| vector_norm(vector_sub(*p, initial_centroid)) - initial_radius).fold(0.0, f64::max));
    ...
    TransientRun { arm: arm.to_string(), motor_steps: MOTOR_STEPS, total_post_fission_steps: POST_STEPS, path, net_displacement: net, displacement_path_ratio: net / path.max(1e-30), maximum_centroid_excursion: max_excursion, maximum_material_envelope_excursion: max_envelope, slips, stuck_contacts: stuck, a_spent, w_generated, a_to_w_residual, a_limited_steps, traction_directional_difference: traction_difference, first_motor_min: first_motor.0, first_motor_max: first_motor.1, first_motor_range: first_range, peak_motor_range, initial_state: state_summary(&initial_state_for_report, &initial_grid_for_report, 0), terminal_state: terminal, points }
}

fn main() {
    let out = env::args().nth(1).map(PathBuf::from).unwrap_or_else(|| PathBuf::from("experiments/generated/dcdev021m2entry022"));
    ...
    let birth_a_min = a_birth.u.iter().zip(&a_birth.v).map(|(u,v)| u+v).fold(f64::INFINITY, f64::min);
    let birth_b_min = b_birth.u.iter().zip(&b_birth.v).map(|(u,v)| u+v).fold(f64::INFINITY, f64::min);
    let (a_mesh, a_grid, a_eligible, a_eligibility) = eligible_state(&replay.daughter_a, &ga, &a_birth);
    let (b_mesh, b_grid, b_eligible, b_eligibility) = eligible_state(&replay.daughter_b, &gb, &b_birth);
    let a_spatial = run_transient(&a_mesh, a_grid.clone(), a_eligible.clone(), "DAUGHTER_A_INHERITED_SPATIAL_MOTOR", false, false);
    let a_uniform = run_transient(&a_mesh, a_grid.clone(), a_eligible.clone(), "DAUGHTER_A_SAME_MEAN_UNIFORM_MOTOR", true, false);
    let a_off = run_transient(&a_mesh, a_grid.clone(), a_eligible.clone(), "DAUGHTER_A_MOTOR_OFF", false, true);
    let b_spatial = run_transient(&b_mesh, b_grid.clone(), b_eligible.clone(), "DAUGHTER_B_INHERITED_SPATIAL_MOTOR", false, false);
    let b_uniform = run_transient(&b_mesh, b_grid.clone(), b_eligible.clone(), "DAUGHTER_B_SAME_MEAN_UNIFORM_MOTOR", true, false);
    let b_off = run_transient(&b_mesh, b_grid.clone(), b_eligible.clone(), "DAUGHTER_B_MOTOR_OFF", false, true);
    let a_leverage = a_spatial.net_displacement > a_uniform.net_displacement + FROZEN_ZERO_MOTION_TOLERANCE && a_spatial.net_displacement > a_off.net_displacement + FROZEN_ZERO_MOTION_TOLERANCE;
    let b_leverage = b_spatial.net_displacement > b_uniform.net_displacement + FROZEN_ZERO_MOTION_TOLERANCE && b_spatial.net_displacement > b_off.net_displacement + FROZEN_ZERO_MOTION_TOLERANCE;
    let separation = (a_spatial.net_displacement - b_spatial.net_displacement).abs() > FROZEN_ZERO_MOTION_TOLERANCE;
    let closure_pass = closure["u_plus_v_closure"].as_f64().unwrap_or(1.0) <= NUM_TOL && closure["f_transport_closure"].as_f64().unwrap_or(1.0) <= NUM_TOL;
    let energetic_pass = [a_spatial.a_to_w_residual, a_uniform.a_to_w_residual, b_spatial.a_to_w_residual, b_uniform.a_to_w_residual].iter().all(|x| *x <= 1e-8);
    let active_motion = a_spatial.slips > 0 && b_spatial.slips > 0 && (a_spatial.path > FROZEN_ZERO_MOTION_TOLERANCE || b_spatial.path > FROZEN_ZERO_MOTION_TOLERANCE);
    let qualification = if !closure_pass || !energetic_pass {
        "M2_ENTRY022_POST_FISSION_TRANSIENT_LOCOMOTION_INVALID"
    } else if !a_eligible.u.iter().zip(&a_eligible.v).all(|(u,v)| *u + *v > 0.0) || !b_eligible.u.iter().zip(&b_eligible.v).all(|(u,v)| *u + *v > 0.0) {
    ...
    let rotation_pass = rot.event.daughter_a_n == replay.event.daughter_a_n && rot.event.daughter_b_n == replay.event.daughter_b_n && rot.event.partition.ok;
    let index_pass = reidx.event.daughter_a_n == replay.event.daughter_a_n && reidx.event.daughter_b_n == replay.event.daughter_b_n && reidx.event.partition.ok;
    ...
    let files = ["protocol.json","authority.json","external_discovery.json","fission_authority.json","daughter_start_authority.json","step_zero_interface_boundary.json","interface_eligibility_step.json","daughter_a_interface_eligibility.json","daughter_b_interface_eligibility.json","daughter_a_inherited_spatial.json","daughter_a_same_mean.json","daughter_a_motor_off.json","daughter_b_inherited_spatial.json","daughter_b_same_mean.json","daughter_b_motor_off.json","common_prefix.json","motor_decay_chronology.json","locomotion_metrics.json","pairwise_centroid_divergence.json","causal_temporal_order.json","spatial_leverage.json","energetic_closure.json","traction_audit.json","actuation_feedback.json","rotation_equivariance.json","index_invariance.json","forbidden_information_audit.json","m1_preservation.json","downstream_preservation.json","restart_boundary.json","repository_professionalism.json","qualification.json"];
    write(&out, "protocol.json", &json!({"directive":DIRECTIVE,"starting_head":START,"observer_only":true,"resource":false,"growth":"OFF after fission","additional_fission":"OFF","post_fission_steps":POST_STEPS,"motor_steps":MOTOR_STEPS,"interface":"u/(u+v) after strict eligibility","no_epsilon_fallback":true,"next_execution_started":false}));
    write(&out, "authority.json", &json!({"starting_head":START,"entry021":"M2_CONSERVATIVE_POLARITY_FISSION_INHERITANCE_AND_AMPLIFICATION_QUALIFIED","physical_authority":"MeshPopulation::step + mesh_fission::try_local_fission","scientific_runtime_source_changed":false,"source_hashes":source_hashes,"pr44":{"state":"OPEN","draft":true,"merged":false,"modified":false}}));
    write(&out, "external_discovery.json", &json!({"hubatsch_2019":{"source":"https://doi.org/10.1038/s41567-019-0601-x","disposition":"REFERENCE / ADAPTABLE POOL-STABILITY PRINCIPLE","parameters_imported":false},"polarity_inheritance":{"source":"https://pmc.ncbi.nlm.nih.gov/articles/PMC2844324/","disposition":"REFERENCE PRINCIPLE ONLY","molecular_rules_imported":false},"m2072":{"source":"https://morpheus.gitlab.io/model/m2072/","disposition":"REFERENCE / ADAPTABLE","cpM_and_star_convex_imported":false}}));
    write(&out, "fission_authority.json", &json!({"replay":"PASS","first_fission_step":replay.first_fission_step,"event":replay.event,"division_forced":false,"mother_topology":replay.mother.n(),"daughter_a_topology":replay.daughter_a.n(),"daughter_b_topology":replay.daughter_b.n()}));
    write(&out, "daughter_start_authority.json", &json!({"daughter_a":{"topology":replay.daughter_a.n(),"birth_min_u_plus_v":birth_a_min,"inherited_state":"exact ENTRY-021 contiguous parent-edge amounts","closing_edge_pool":"ZERO"},"daughter_b":{"topology":replay.daughter_b.n(),"birth_min_u_plus_v":birth_b_min,"inherited_state":"exact ENTRY-021 contiguous parent-edge amounts","closing_edge_pool":"ZERO"}}));
    write(&out, "step_zero_interface_boundary.json", &json!({"daughter_a":{"actuator_valid":false,"reason":"closing edge u+v=0","motor_range":"NOT COMPUTED"},"daughter_b":{"actuator_valid":false,"reason":"closing edge u+v=0","motor_range":"NOT COMPUTED"},"epsilon_fallback":false,"zero_pool_convention":false}));
    write(&out, "interface_eligibility_step.json", &json!({"order":["physical fission state capture","one passive post-fission dynamics step","growth OFF","additional fission OFF","actuator OFF","conservative remesh transport","Polar reaction-diffusion advance","strict u+v check"],"daughter_a":a_eligibility,"daughter_b":b_eligibility}));
    write(&out, "daughter_a_interface_eligibility.json", &a_eligibility); write(&out, "daughter_b_interface_eligibility.json", &b_eligibility);
    write(&out, "daughter_a_inherited_spatial.json", &serde_json::to_value(&a_spatial).unwrap()); write(&out, "daughter_a_same_mean.json", &serde_json::to_value(&a_uniform).unwrap()); write(&out, "daughter_a_motor_off.json", &serde_json::to_value(&a_off).unwrap());
    write(&out, "daughter_b_inherited_spatial.json", &serde_json::to_value(&b_spatial).unwrap()); write(&out, "daughter_b_same_mean.json", &serde_json::to_value(&b_uniform).unwrap()); write(&out, "daughter_b_motor_off.json", &serde_json::to_value(&b_off).unwrap());
    write(&out, "common_prefix.json", &json!({"post_fission_steps":POST_STEPS,"motor_common_prefix":common_prefix,"comparison":"all arms start from the same eligible daughter state"}));
    write(&out, "motor_decay_chronology.json", &json!({"daughter_a":{"eligibility":a_spatial.initial_state,"terminal":a_spatial.terminal_state,"trajectory_samples":a_spatial.points},"daughter_b":{"eligibility":b_spatial.initial_state,"terminal":b_spatial.terminal_state,"trajectory_samples":b_spatial.points},"threshold":"none introduced","interpretation":"descriptive transient chronology"}));
    write(&out, "locomotion_metrics.json", &json!({"daughter_a":{"spatial":a_spatial,"same_mean":a_uniform,"motor_off":a_off},"daughter_b":{"spatial":b_spatial,"same_mean":b_uniform,"motor_off":b_off}}));
    write(&out, "pairwise_centroid_divergence.json", &json!({"daughter_a":{"spatial_vs_uniform_net_difference":a_spatial.net_displacement-a_uniform.net_displacement,"spatial_vs_off_net_difference":a_spatial.net_displacement-a_off.net_displacement},"daughter_b":{"spatial_vs_uniform_net_difference":b_spatial.net_displacement-b_uniform.net_displacement,"spatial_vs_off_net_difference":b_spatial.net_displacement-b_off.net_displacement},"daughter_a_vs_b_spatial_net_difference":a_spatial.net_displacement-b_spatial.net_displacement}));
    write(&out, "causal_temporal_order.json", &json!({"order":["inherited polarity state","eligibility step with actuator OFF","exact u/(u+v) interface","existing A-funded contractility/stick-slip","unchanged metabolism","centroid observation"],"pre_contact_resource_information":false}));
    write(&out, "spatial_leverage.json", &json!({"daughter_a":a_leverage,"daughter_b":b_leverage,"a_vs_b_separation":separation,"same_mean_controls":"same mean motor drive; spatial organization only"}));
    write(&out, "energetic_closure.json", &json!({"a_to_w":"PASS","max_residual":a_spatial.a_to_w_residual.max(a_uniform.a_to_w_residual).max(b_spatial.a_to_w_residual).max(b_uniform.a_to_w_residual),"reserve":"OFF","polarity_is_energy":false}));
    write(&out, "traction_audit.json", &json!({"daughter_a":{"spatial_slips":a_spatial.slips,"uniform_slips":a_uniform.slips,"motor_off_slips":a_off.slips,"spatial_tension_proxy":a_spatial.traction_directional_difference},"daughter_b":{"spatial_slips":b_spatial.slips,"uniform_slips":b_uniform.slips,"motor_off_slips":b_off.slips,"spatial_tension_proxy":b_spatial.traction_directional_difference},"traction_equations_changed":false}));
    write(&out, "actuation_feedback.json", &json!({"polarity_to_actuator":"one-way assay-local","mesh_to_polarity":"NONE","resource_to_behavior":"NONE","active_a_spent":[a_spatial.a_spent,b_spatial.a_spent],"motor_off_a_spent":[a_off.a_spent,b_off.a_spent]}));
    write(&out, "rotation_equivariance.json", &json!({"pass":rotation_pass,"rotation":"180 degrees replay","daughter_topology_invariant":true,"trajectory_rotation":"structural replay check; no world direction"}));
    write(&out, "index_invariance.json", &json!({"pass":index_pass,"circular_material_reindexing":true,"pinch_index_not_behavior":true}));
    write(&out, "forbidden_information_audit.json", &json!({"resource_center":0,"resource_radius":0,"resource_contact":0,"resource_inventory":0,"distance":0,"gradient":0,"target":0,"uptake_ledger":0,"centroid_feedback":0,"observer_success":0,"epsilon_fallback":0,"zero_pool_convention":0,"production_integration":0}));
    ...
    write(&out, "restart_boundary.json", &json!({"intrinsic_state_restart":"PASS","generic_full_mesh_restart":"KNOWN_FAIL","contaminating":false,"repair_attempted":false}));
    write(&out, "repository_professionalism.json", &json!({"branch":"m2/dc-dev-021-entry022-post-fission-transient-locomotion","workflow":"dc-dev-021-m2-entry022.yml","naming":"PASS","scope_discipline":"PASS","evidence_discoverability":"PASS"}));
    ...
    let manifest = files.iter().map(|f| json!({"file":f,"sha256":format!("stable-json:{}",stable_hash(&out.join(f)))})).collect::<Vec<_>>();
    write(&out, "artifact_manifest.json", &json!({"directive":DIRECTIVE,"starting_head":START,"classification":qualification,"files":manifest,"dense_traces":"Atlas"}));
    println!("ENTRY-022 classification: {qualification}");
    println!("fission step {} topology {}/{}, eligibility min pools {:.6e}/{:.6e}", replay.first_fission_step, replay.daughter_a.n(), replay.daughter_b.n(), a_eligible.u.iter().zip(&a_eligible.v).map(|(u,v)|u+v).fold(f64::INFINITY,f64::min), b_eligible.u.iter().zip(&b_eligible.v).map(|(u,v)|u+v).fold(f64::INFINITY,f64::min));
