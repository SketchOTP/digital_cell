//! DC-DEV-020-M1-REPLAN-002 diagnostic maturation-coupled load-bearing.
//!
//! This example is deliberately a shadow.  The production reaction, mechanics,
//! remesh, transport, and death code are called unchanged.  The age ledger
//! transfers newly built M into a young pool before applying the existing
//! k_turn maturation and turnover equations.  The D arm additionally derives
//! its diagnostic load-bearing reference from mature M only; it never changes
//! production defaults or serialized organism state.
...
use chemistry_core::mesh_reactions::{
    apply_structural_damage, reactions_step, reactions_step_with_reference_lengths,
    try_local_rebond, ReactionLedger, ReactionParams,
};
...
const DIRECTIVE: &str = "DC-DEV-020-M1-REPLAN-002-MATURATION-COUPLED-LOAD-BEARING-FEASIBILITY-001";
const STARTING_HEAD: &str = "92075021ae1f4c9917f7ace9b160e5694e001de2";
const DT: f64 = 0.02;
...
const ATLAS_DENSE_ROOT: &str =
    r"\\atlas\ATLAS\100_ACTIVE\Projects\DIGITAL_CELL\evidence\dcdev020m1replan002";
...
struct State {
    ...
    age_mature: f64,
    age_total: f64,
    mature_reference_perimeter: f64,
    mean_strain: f64,
    mean_turn_scale: f64,
...
impl AgePools {
    ...
    /// Diagnostic load-bearing reference: only mature material defines the
    /// mechanical/strain rest length.  The existing 1e-15 bound is used only
    /// to keep zero-mature edges numerically well-defined; it is not a
    /// biological minimum body size or material floor.
    fn mature_reference_lengths(&self, mesh: &MaterialMesh) -> Result<Vec<f64>, String> {
        if self.mature.len() != mesh.n() || !self.valid_for(mesh) {
            return Err("mature reference topology/identity is invalid".into());
        }
        Ok(self
            .mature
            .iter()
            .map(|mature| (mature.max(1e-15) / mesh.rho_s.max(1e-15)).max(1e-15))
            .collect())
    }

    fn mature_reference_perimeter(&self, mesh: &MaterialMesh) -> Result<f64, String> {
        Ok(self.mature_reference_lengths(mesh)?.iter().sum())
    }
}
...
impl Mode {
    fn id(self) -> &'static str {
        match self {
            ...
            Self::MaturationLoadBearing => "MOVING_MATURATION_LOAD_BEARING_SHADOW",
        }
    }
...
fn age_correct_reaction(
    ...
    for i in 0..mesh.n() {
        ...
        let l0 = reference_lengths
            ...
            .unwrap_or_else(|| before.rest_length(i));
        let reference_strain = (before.edge_length(i) - l0) / l0;
        let q = (params.k_turn * (1.0 / (1.0 + 2.0 * reference_strain.max(0.0))) * dt)
            .clamp(0.0, 1.0 - 1e-15);
        ...
        let scale = 1.0 / (1.0 + 2.0 * reference_strain.max(0.0));
...
fn state(
    ...
    let (young, mature, total, mature_reference_perimeter) =
        pools.map_or((0.0, 0.0, 0.0, 0.0), |p| {
            (
                p.young.iter().sum(),
                p.mature.iter().sum(),
                p.total(),
                p.mature_reference_perimeter(mesh).unwrap_or(0.0),
            )
        });
    State {
        step,
...
fn run_arm(
    ...
    let age_structured = matches!(mode, Mode::AgeStructured | Mode::MaturationLoadBearing);
    ...
    for step in 1..=steps {
        ...
        let reference_lengths = if matches!(mode, Mode::MaturationLoadBearing) {
            Some(
                pools
                    .as_ref()
                    .ok_or_else(|| "maturation candidate has no age pools".to_string())?
                    .mature_reference_lengths(&mesh)?,
            )
        } else {
            None
        };
        let mut ledger = if let Some(references) = reference_lengths.as_deref() {
            reactions_step_with_reference_lengths(&mut mesh, &reaction, DT, true, true, references)
        } else {
        ...
        if mode.moving() {
            ...
            let mechanics_ok = if matches!(mode, Mode::MaturationLoadBearing) {
                let references = pools
                    .as_ref()
                    .ok_or_else(|| "maturation candidate lost age pools".to_string())?
                    .mature_reference_lengths(&mesh)?;
                mechanics_step_with_reference_lengths(&mut mesh, &mechanics, &references)
            } else {
...
mod tests {
    ...
    #[test]
    fn mature_reference_is_material_derived_and_zero_safe() {
        let (mut mesh, _) = r5_entry::m1r1_entry_state();
        mesh.stamp_geometry_conservative_schema();
        let mut pools = AgePools::from_mesh(&mesh);
        pools.mature[0] = 0.0;
        pools.young[0] = mesh.edges[0].m;
        let references = pools
            .mature_reference_lengths(&mesh)
            .expect("reference lengths");
        assert!(references
            .iter()
            .all(|length| length.is_finite() && *length > 0.0));
        assert!(close(references[1], pools.mature[1] / mesh.rho_s));
        assert!(references[0] <= 1.0e-14);
    }
...
fn main() -> Result<(), String> {
    let out = std::env::var_os("DCDEV020M1REPLAN002_OUTPUT")
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from("experiments/generated/dcdev020m1replan002"));
    let dense = std::env::var_os("DCDEV020M1REPLAN002_DENSE_OUTPUT")
        .map(PathBuf::from)
    ...
    let (maturation_arm, _, _, _) = run_arm(
        &entry,
        None,
        Mode::MaturationLoadBearing,
        Some(&sealed_schedule),
        HORIZON,
        "arm_d_maturation_load_bearing",
        dense.as_deref(),
    )?;
    let (deprived_current, deprived_current_mesh, _, _) = run_arm(
        &entry,
        None,
        Mode::Current,
        Some(&[]),
        DEPRIVATION,
        "arm_b_deprivation_replay",
        dense.as_deref(),
    )?;
    let (recovered_current, _, _, _) = run_arm(
        &deprived_current_mesh,
        None,
        Mode::Current,
        Some(&sealed_schedule),
        HORIZON,
    ...
    let (deprived_maturation, deprived_maturation_mesh, deprived_maturation_pools, _) = run_arm(
        &entry,
        None,
        Mode::MaturationLoadBearing,
        Some(&[]),
        DEPRIVATION,
        "arm_d_deprivation_replay",
        dense.as_deref(),
    )?;
    let (recovered_maturation, _, _, _) = run_arm(
        &deprived_maturation_mesh,
        deprived_maturation_pools.as_ref(),
        Mode::MaturationLoadBearing,
        Some(&sealed_schedule),
        HORIZON,
        "arm_d_recovery",
    ...
    let (starved, _, _, _) = run_arm(
        ...
        Some(&[]),
        STARVATION_BOUND,
        "arm_d_zero_resource_starvation",
        dense.as_deref(),
    )?;
    ...
    let controls_reproduce = close(static_arm.organized_material_delta, 0.34214067689040917)
        ...
        && close(age_arm.organized_material_delta, -1.7633945824468071);
    let candidate_homeostasis = maturation_arm.organized_material_delta >= -TOLERANCE;
    let recovery_pass = |deprived: &ArmResult, recovered: &ArmResult, baseline: &ArmResult| {
        recovered.final_state.organized_material > deprived.final_state.organized_material
            && (recovered.final_state.organized_material - baseline.initial.organized_material)
                .abs()
                < (deprived.final_state.organized_material - baseline.initial.organized_material)
                    .abs()
    };
    let recovery_current = recovery_pass(&deprived_current, &recovered_current, &current_arm);
    let recovery_age = recovery_pass(&deprived, &recovered, &age_arm);
    let recovery_candidate =
        recovery_pass(&deprived_maturation, &recovered_maturation, &maturation_arm);
    let all_recoveries = recovery_current && recovery_age && recovery_candidate;
    ...
    let resource_delivery_same = close(maturation_arm.n_delivered, age_arm.n_delivered)
        && close(maturation_arm.f_delivered, age_arm.f_delivered)
        && maturation_arm.source_schedule_hash == age_arm.source_schedule_hash;
    let reference_identity = maturation_arm.closure.max_age_identity_residual <= TOLERANCE
        && maturation_arm
            .initial
            .mature_reference_perimeter
            .is_finite()
        && maturation_arm
            ...
            .mature_reference_perimeter
            .is_finite()
        && maturation_arm.initial.mature_reference_perimeter > 0.0;
    let boundedness = [
        maturation_arm.initial.mature_reference_perimeter,
        maturation_arm.final_state.mature_reference_perimeter,
        maturation_arm.initial.age_young,
        maturation_arm.final_state.age_young,
        maturation_arm.initial.age_mature,
        maturation_arm.final_state.age_mature,
        maturation_arm.final_state.structural_m,
    ]
    .into_iter()
    .all(f64::is_finite);
    let closure = static_arm.closure.pass()
        ...
        && maturation_arm.closure.pass()
        && deprived_current.closure.pass()
        && recovered_current.closure.pass()
        && deprived.closure.pass()
        && recovered.closure.pass()
        && deprived_maturation.closure.pass()
        && recovered_maturation.closure.pass()
    ...
    let classification = if !closure
        || !damage
        || !controls_reproduce
        || !resource_delivery_same
        || !reference_identity
        || !boundedness
        || !d087
    {
        "M1_MATURATION_COUPLED_LOAD_BEARING_INVALID"
    } else if !candidate_homeostasis {
    ...
    let protocol = json!({
        ...
        "entry_authority": {"replan001_classification": "M1_AGE_STRUCTURED_TURNOVER_INSUFFICIENT", "production_default": "ConservativeV2 / reserve OFF"},
        ...
        "age_contract": {"entry": "all existing M mature", "build": "new M young", "maturation": "min(M_young, k_turn * M_young * dt)", "turnover": "existing strain-scaled k_turn on mature only", "new_timescale": false, "physical_material": "edge.m = M_young + M_mature"},
        "mature_load_bearing_reference": {"reference_length": "M_mature / rho_s", "zero_guard": "existing 1e-15 numerical division bound only", "used_for": ["stretch force rest length", "structural build strain", "structural turnover strain"], "not_used_for": ["physical edge material", "rupture accounting", "target geometry"]},
        "arms": ["A_frozen_geometry_current", "B_moving_current", "C_moving_age_structured", "D_moving_maturation_load_bearing", "B/C/D_deprivation_recovery", "D_zero_resource_starvation"],
        "observer_only": true,
        "forbidden_changes": ["production biology", "coefficients", "production defaults", "source schedule", "transport", "resource redesign", "target size", "controller", "reserve", "recycling", "salvage", "M2", "DC-DEV-021", "REPLAN-003"],
        "next_execution_started": false
    });
    let results = json!({
        "directive": DIRECTIVE,
        "starting_head": STARTING_HEAD,
        "arms": {"A_static_current": static_arm, "B_moving_current": current_arm, "C_moving_age_structured": age_arm, "D_maturation_load_bearing": maturation_arm, "D_starvation": starved},
        "recovery": {"B": {"deprived": deprived_current, "refed": recovered_current, "pass": recovery_current}, "C": {"deprived": deprived, "refed": recovered, "pass": recovery_age}, "D": {"deprived": deprived_maturation, "refed": recovered_maturation, "pass": recovery_candidate}, "candidate_pass": recovery_candidate, "all_control_and_candidate_pass": all_recoveries},
        "checks": {"controls_reproduce": controls_reproduce, "candidate_homeostasis": candidate_homeostasis, "candidate_recovery": recovery_candidate, "all_recoveries": all_recoveries, "starvation_decline": starvation_decline, "physical_loss": physical_loss, "damage_preservation": damage, "material_closure": closure, "d087": d087, "age_identity": reference_identity, "reference_identity": reference_identity, "boundedness": boundedness, "resource_delivery_same": resource_delivery_same},
    ...
    let qualification = json!({
        "directive": DIRECTIVE,
        "e0_authority": true,
        "e1_material_identity_and_lineage": reference_identity && damage,
        "e2_fed_moving_feasibility": controls_reproduce && candidate_homeostasis,
        "e3_no_reset_recovery": recovery_candidate,
        "e4_starvation_degradation": starvation_decline,
    ...
    let preservation = json!({
        ...
        "replan001_age_control_reproduction": close(age_arm.organized_material_delta, -1.7633945824468071),
        "maturation_load_bearing_candidate": candidate_homeostasis,
        "maturation_recovery": recovery_candidate,
        "mature_reference_identity": reference_identity,
        "mature_reference_bounded": boundedness,
        "resource_delivery_parity": resource_delivery_same,
        "gc_material_closure": closure,
        "v2_d087": d087_pass(&d087_v2, "ConservativeV2"),
        "v3_d087": d087_pass(&d087_v3, "ConservativeV3"),
        "age_state_identity": reference_identity,
        "remesh_age_lineage": maturation_arm.closure.max_age_identity_residual <= TOLERANCE,
    ...
    write_json(
        ...
        &json!({"schema": "dcdev020m1replan002_manifest_v1", "directive": DIRECTIVE, "starting_head": STARTING_HEAD, "files": ["protocol.json", "results.json", "qualification.json", "preservation.json", "artifact_manifest.json"], "dense_output": ATLAS_DENSE_ROOT, "shared_drive_required": true, "sha256": "computed-by-workflow"}),
    )?;
    println!("DCDEV020M1REPLAN002_MATURATION_COUPLED_LOAD_BEARING_FEASIBILITY_COMPLETE");
    ...
    println!("maturation_recovery={recovery_candidate}");
    println!("all_control_and_candidate_recovery={all_recoveries}");
