const C10_DIRECTIVE: &str =
    "DC-DEV-021-M2-CLOSURE-010-COMBINED-LOCAL-CONTACT-MATERIAL-WORK-REPRODUCTION-QUALIFICATION-001";
const C10_START: &str = "011c3b5c7404ca453cb1546eee6271f1bf3af538";
const C10_STEPS: usize = 12_000;
const C10_TOL: f64 = 1e-10;

fn c10_combined_mechanics(
    ...
    regulator
        .step(frame, event)
        .map_err(|e| format!("combined regulator continuity: {e}"))?;
    let raw = entry025_anti(&agent.polarity);
    let clutch = entry025_direct(&agent.polarity);
    if raw.len() != regulator.state.activity.len()
        || clutch.len() != raw.len()
        || contact.len() != raw.len()
    {
    ...
    let ledger =
        ...
        .map_err(|e| format!("combined material/contact motor: {e:?}"))?;
    if !agent.mesh.physical_runtime_valid() || !agent.mesh.lifecycle_invariants_hold() {
        return Err("invalid mesh after combined mechanics".into());
    }
...
fn c10_combined_run(
    ...
    for step in 1..=C10_STEPS {
        if agents.is_empty() {
            break;
        }
        let old_positions: Vec<_> = agents.iter().map(closure002_point).collect();
        let contact_vectors: Vec<Vec<f64>> = agents
            .iter()
            .map(|agent| {
                if contact_feedback {
                    c6_contact_signal(&world, &agent.mesh)
                } else {
                    vec![0.0; agent.mesh.n()]
                }
            })
