...
Qualification requires a declared `SelectivePressureContractV1`. Treatment replicates must observe that exact declared intervention before reproduction; neutral replicates are validated as controls without treating unrelated environmental activity as pressure. Campaign contracts must identify the same contrast while declaring treatment and neutral roles explicitly.
